...
{
    if constexpr (can_be_initialized<T,
                                     any_type,
                                     any_type,
                                     any_type,
                                     any_type,
        ...
        can_be_initialized<T, any_type, any_type, any_type, any_type, any_type, any_type, any_type>)
        return 7;
    else if constexpr (
        can_be_initialized<T, any_type, any_type, any_type, any_type, any_type, any_type>)
        return 6;
...
// Primitives - string
template <> inline std::string serialize<std::string>(const std::string& value, int)
{
    return "\"" + value + "\"";
}
...
// Vectors
template <typename T> std::string serialize(const std::vector<T>& vec, int indent)
{
    if (vec.empty())
        return "[]";
...
template <typename T>
    requires(!std::is_enum_v<T> && std::is_aggregate_v<T> && !std::is_same_v<T, std::string>)
...
int main()
{
    ...
    Project p{"MyProject", {{"Fix bug", Status::Active, 1}, {"Add feature", Status::Pending, 2}}};
